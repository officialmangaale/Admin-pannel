...
import { billingApi, BillingPlan, BillingSummary } from "@/lib/api";
...
export default function SwitchPlanModal({ isOpen, onClose, restaurantId, currentSummary, onSuccess, showToast }: Props) {
    ...
    const currentPlanName = currentSummary?.current_plan?.name || "None";
    const currentPlanType = currentSummary?.plan_type || currentSummary?.current_plan?.type;
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            await billingApi.switchPlan(restaurantId, {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Switch Billing Plan" maxWidth="lg">
            <div className="space-y-6">
                {/* Current Plan Info */}
                <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-100">
                    <p className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-1">Current Plan</p>
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-slate-800">{currentPlanName}</span>
                        {currentPlanType && (
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                currentPlanType === "PER_ORDER" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"
                            }`}>
                                {currentPlanType}
                            </span>
                        )}
                ...
                {/* New Plan Selection */}
                <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Switch to *</label>
                    {loadingPlans ? (
                ...
                {selectedPlan && (
                    <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">New Plan Details</p>
                ...
                <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button
                        ...
                        className="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm flex items-center gap-2"
                    >
